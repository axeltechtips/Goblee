...
class GoveeDevice:
    ...
    async def getUUID(self, target_handle):
        async with BleakClient(self.mac) as client:
            # Deprecated warning workaround: access services property
    ...
    async def writeToDevice(self, handle, value):
        ...
        if theUUID is None:
            raise RuntimeError(f"Characteristic UUID for handle {handle:#04x} not found.")
        ...
        try:
            if status is True:
                out = loop.run_until_complete(
                    self.writeToDevice(0x0013, "3301010000000000000000000000000000000033")
                )
                return True, status, out
            elif status is False:
                out = loop.run_until_complete(
                    self.writeToDevice(0x0013, "3301000000000000000000000000000000000032")
                )
                return True, status, out
            else:
    ...
    def setColor(self, c: typing.List[int], segment: typing.List[int] = None):
        ...
        for v in c:
            ...
            if not (0 <= v <= 255):
                raise ValueError
        hex_c = [hex(v) if len(hex(v)) == 4 else hex(v)[:2] + "0" + hex(v)[2:] for v in c]
        l = hex(l) if len(hex(l)) == 4 else hex(l)[:2] + "0" + hex(l)[2:]
        r = hex(r) if len(hex(r)) == 4 else hex(r)[:2] + "0" + hex(r)[2:]
        ...
        asyncio.set_event_loop(loop)
        try:
            out = loop.run_until_complete(self.writeToDevice(0x0013, cs_p))
            return True, c, out
        finally:
            loop.close()
    ...
    def setBrightness(self, level: int):
        ...
        """
        if level < 7 and level != 0:  # Fix weird issue with level <7
            level = 7
        if level == 0:
    ...
    def setScene(self, setting):
        ...
        try:
            out = loop.run_until_complete(
                self.writeToDevice(0x0013, scenes[setting.lower()])
            )
    ...
    def setColorMusic(self, setting, c=[255, 0, 0]):
        ...
        asyncio.set_event_loop(loop)
        try:
            out = loop.run_until_complete(self.writeToDevice(0x0013, cur_setting))
